const { Artifacts, Booking } = require("../Models/index");
...
module.exports.createProperty = async (req, res) => {
    try {
        ...
        return res.status(201).json({
            ...
            property: newArtifact,
        });
    } catch (error) {
        console.error("Error adding artifact:", error.message);
...
module.exports.getSpecificProperty = async (req, res) => {
    try {
        if (req.params.id) {
            ...
            const hostProperties = await Artifacts.findOne({
                where: { id: ArtifactId }
            });
            ...
            // Find all properties where the logged-in user is the host
            const hostProperties = await Artifacts.findAll({
                where: { hostId }
            });
